...
The complete 38-file mapping is recorded in
...
| Workflow entrypoints | 7 | `references/workflows/<plugin>/SKILL.md` |
| Workflow support resources | 29 | `references/workflows/<plugin>/{references,assets,scripts}/...` |

The support-resource set is derived mechanically from every file under each
plugin's `references/`, `assets/`, and `scripts/` directories; plugin manifests
under `.claude-plugin/` are excluded. Per-workflow directories prevent name
collisions and preserve every relative dependency from its original
`plugins/<plugin>/` layout. Copy all 38 canonical sources to their exact
installed paths, compare all 38 source/install SHA-256 digests, and validate
that every local Markdown dependency resolves in both layouts.

The 7 tracked entrypoint route stubs live at
`codex/overnight-workflows/references/workflows/<plugin>/SKILL.md`. They are
repository navigation files, not install sources; validate that each resolves
to its canonical plugin entrypoint.
