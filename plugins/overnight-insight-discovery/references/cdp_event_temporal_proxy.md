# Recipe — CDP-event Tier 1b temporal proxy
...
declaring the candidate Tier 3 (blocked), check whether a CDP event
...
`your-project.cdp_events_export.file_upload` (~28K events, 3,122
...
2. **Find the BR event.** Query the cdp_events_export dataset for
   event names matching the semantic. Common candidates:
   `file_upload` · `step_complete` · `form_submit` · `page_visit` ·
   `campaign_response`.
3. **Verify native `TIMESTAMP`.** `bq show your-project.cdp_events_export.{event}`
...
4. **Verify join key.** `salesforce_account_id` (BR side) →
   `LOWER(crm_account.id)` (SF side). Always `LOWER()`-normalise
...
`docs/sf_cdp_event_proxy_matrix.md` in the project repo. Summary:
