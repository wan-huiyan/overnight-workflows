...
Salesforce BigQuery exports for the client are **latest-snapshot only** (per
...
1. **Primary stratifier** uses an SF-snapshot-gated feature → **demote to
   ...
   (see `cdp_event_temporal_proxy.md`).
...
- `cdp_event_temporal_proxy.md` — Tier 1b BR-event proxy workflow.
